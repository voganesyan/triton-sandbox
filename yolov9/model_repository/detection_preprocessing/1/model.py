...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            preprocessing_params = np.expand_dims(np.array(image.shape), axis=0)

            image_preprocessed = pb_utils.Tensor(
                "preprocessed_image", image_preprocessed.astype(self.preprocessed_image_dtype)
            )
            preprocessing_params = pb_utils.Tensor(
                "preprocessing_params", preprocessing_params.astype(self.preprocessing_params_dtype)
            )
